...
export function syncArticleMCQs(
  ...
  (async () => {
    try {
      // Look up the Supabase article ID from firestore_id
...
export function syncQuestionSetCreate(firestoreId: string, data: Record<string, any>) {
  const sb = getSupabase();
  if (!sb) return;
...
export function syncQuestionSetDelete(firestoreId: string) {
  const sb = getSupabase();
  if (!sb) return;
...
export function syncQuestionSetTogglePublish(firestoreId: string, isPublished: boolean) {
  const sb = getSupabase();
  if (!sb) return;
...
export function syncPracticeQuestionCreate(
  ...
  (async () => {
    try {
      // Look up the Supabase question_set ID from firestore_id
